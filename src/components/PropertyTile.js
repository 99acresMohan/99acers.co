...
const PropertyTile = ({ property, refreshList }) => {
    ...
    const handleVerifyOtp = async () => {
        if (otp === '1234') {
            try {
                const response = await axios.post('http://localhost:5000/api/register-visitor', {
                    mobile: mobile,
                    propertyId: property._id
                });
                if (response.data.success) {
                    setSellerData({ name: response.data.sellerName, phone: response.data.sellerPhone });
                    setStep(3);
                }
            } catch (err) { alert("Server Error on Port 5000"); }
        } else { alert("Use 1234"); }
    };
        ...
        <div style={styles.tile}>
            <div style={styles.imageColumn}>
                <div style={styles.imageSection}>
                    {property.imageUrl ? (
                        <img src={property.imageUrl} alt={property.title} style={styles.propertyImg} />
                    ) : (
                        <div style={styles.placeholder}>No Image Available</div>
                    )}
                    <div style={styles.dateTag}>Verified</div>
                </div>
                <div style={styles.postedUnderImage}>
                    <div style={styles.postedText}>📅 Posted: {new Date(property.postedAt).toLocaleDateString('en-GB')}</div>
                    <div style={styles.postedText}>👤 By: <span style={{ color: '#0078db' }}>{property.postedBy || 'Owner'}</span></div>
                </div>
            </div>

            <div style={styles.details}>
                <div style={styles.headerRow}>
                    <h3 style={styles.price}>₹{formatPrice(property.price)}</h3>
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                        <span style={styles.propIdBadge}>ID: {property.propertyId || '...'}</span>
                ...
                <p style={styles.loc}>📍 {property.location}</p>
                ...
                <div style={styles.infoRow}>
                    <div style={styles.infoItem}>
                        ...
                        <span style={styles.val}>
                            ...
                            <span style={styles.unitTag}>
                                {/* REPAIRED LOGIC BELOW: Prioritize areaUnit if it exists */}
                    ...
                    <div style={styles.infoItem}>
                        <span style={styles.label}>Road:</span>
                        <span style={styles.val}>{property.road || 'N/A'}</span>
                    </div>

                    <div style={styles.infoItem}>
                        <span style={styles.label}>Facing:</span>
                        <span style={styles.val}>{property.facing || 'N/A'}</span>
                    </div>

                    <div style={styles.infoItem}>
                        <span style={styles.label}>Corner:</span>
                        <span style={styles.val}>{property.isCorner || 'No'}</span>
                    </div>

                    {property.propertyType !== 'Plot' && (
                        <div style={styles.infoItem}>
                            <span style={styles.label}>Floor:</span>
                            <span style={styles.val}>{property.floor || 'N/A'}</span>
                        </div>
                    )}
                </div>

                <div style={styles.actions}>
                    <button style={styles.viewBtn} onClick={() => setShowFullDesc(!showFullDesc)}>
                        {showFullDesc ? "Hide Details" : "View Details"}
                    </button>
                    <button onClick={() => setShowModal(true)} style={styles.contactBtn}>
                        {sellerData ? `Call ${sellerData.name}` : "Contact Seller"}
                    </button>
                    {isAdmin && <button onClick={handleDelete} style={styles.deleteBtn}>🗑 Delete</button>}
                </div>
                        ...
                        <div style={styles.modal}>
                            {step === 1 && (
                                <div>
                                    ...
                                    <input type="text" placeholder="Mobile Number" style={styles.input} value={mobile} onChange={(e) => setMobile(e.target.value)} />
                                    <button onClick={() => setStep(2)} style={styles.mainBtn}>Get OTP</button>
                                </div>
                            )}
                            {step === 2 && (
                                <div>
                                    <h3>Verify OTP</h3>
                                    <input type="text" placeholder="Enter 1234" style={styles.input} value={otp} onChange={(e) => setOtp(e.target.value)} />
                                    <button onClick={handleVerifyOtp} style={styles.mainBtn}>Verify</button>
                                </div>
                            )}
                            {step === 3 && (
                                <div style={{ textAlign: 'center' }}>
                                    ...
                                    <button onClick={() => setShowModal(false)} style={styles.mainBtn}>Close</button>
                                </div>
                            )}
                            {step !== 3 && <button onClick={() => setShowModal(false)} style={styles.closeBtn}>Cancel</button>}
                        </div>
                    </div>
                )}
            </div>
...
const styles = {
    ...
    unitTag: { marginLeft: '5px', fontSize: '0.7rem', background: '#eee', padding: '2px 5px', borderRadius: '3px', textTransform: 'uppercase' }
};
